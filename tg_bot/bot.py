    ...
    if action == "consent":
        if not consent:
            ...
            text = (
                "Мини-оферта:\n\n"
                "Вы соглашаетесь на обработку персональных данных, включая ваш Telegram-идентификатор и ссылку "
                "на профиль участника сайта «5 вёрст», исключительно для обеспечения работы персонализированных "
                "функций на сайте run5k.run.\n\n"
                "Собранные данные не передаются третьим лицам и не используются вне рамок проекта run5k.run. "
                "Допускается отправка информационных сообщений и уведомлений."
            )
